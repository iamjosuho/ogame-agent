...
### 兩種 Quick Start 都共用的最後一步：建立 AI Agent 的定期巡邏排程（要自動化就必做）

不論你選擇方式 A 或方式 B，`init` 與 `matrix` 都只會完成專案初始化與連線驗證，**不會**自動建立或啟用定期任務。若要讓巡邏自動執行，請在所選 AI Agent 的排程介面中建立定期任務、定期提示詞或 Cron 型的 Agent 喚醒排程，並設定：

1. **Working directory / project：**這個 clone 的 `ogame-agent` repository。
2. **Frequency：**依你的遊戲節奏調整；可先從每小時一次開始。
3. **Prompt：**[`.agent/skills/ogame-patrol/CRONJOB_PROMPT.md`](.agent/skills/ogame-patrol/CRONJOB_PROMPT.md)。複製到排程中，並依所選 AI Agent 的需求自行調整。
